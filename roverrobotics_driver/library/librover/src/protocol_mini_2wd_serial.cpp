...
Mini2WDProtocolObject::Mini2WDProtocolObject(const char *device,
  ...
  // Clear status and apply angular scaling
  ...
  // Motion controller (diff drive; 2 wheels)
  ...
  // Threads
  write_to_robot_thread_= std::thread([this]() { this->send_getvalues_command(10); });
  motor_speed_update_thread_ = std::thread([this]() { this->motors_control_loop(30); });
  std::cerr << "protocol is running..." << std::endl;
}

void Mini2WDProtocolObject::load_persistent_params() {
...
void Mini2WDProtocolObject::update_drivetrim(double delta) {
...
void Mini2WDProtocolObject::send_estop(bool estop) {
  robotstatus_mutex_.lock();
  estop_ = estop;
  robotstatus_mutex_.unlock();
}

robotData Mini2WDProtocolObject::status_request() { return robotstatus_; }

robotData Mini2WDProtocolObject::info_request() { return robotstatus_; }

void Mini2WDProtocolObject::set_robot_velocity(double *controlarray) {
  robotstatus_mutex_.lock();
...
void Mini2WDProtocolObject::motors_control_loop(int sleeptime) {
  ...
  while (true) {
    ...
    rpm_BL = 0; //robotstatus_.motor1_rpm / MOTOR_RPM_TO_WHEEL_RPM_RATIO_;
    rpm_BR = 0; //robotstatus_.motor2_rpm / MOTOR_RPM_TO_WHEEL_RPM_RATIO_;
...
void Mini2WDProtocolObject::unpack_comm_response(std::vector<uint8_t> robotmsg) {
...
bool Mini2WDProtocolObject::is_connected() { return comm_base_->is_connected(); }

int Mini2WDProtocolObject::cycle_robot_mode() {
...
void Mini2WDProtocolObject::register_comm_base(const char *device) {
...
void Mini2WDProtocolObject::send_getvalues_command(int sleeptime) {
  while (true) {
    if (comm_type_ == "serial") {
...
void Mini2WDProtocolObject::send_motors_commands() {
  robotstatus_mutex_.lock();
...
unsigned short Mini2WDProtocolObject::crc16(unsigned char *buf,
                                          unsigned int len) {
  unsigned int i;
