...
motor_data computeMecanumWheelSpeeds(robot_velocities target_velocities,
                                       robot_geometry robot_geometry) {
  float vx = target_velocities.linear_velocity;
  float vy = target_velocities.trans_velocity;
  float vw = target_velocities.angular_velocity;

  float lx = robot_geometry.intra_axle_distance / 2.0;
  float ly = robot_geometry.wheel_base / 2.0;
...
robot_velocities computeMecanumVelocitiesFromWheelspeeds(
    motor_data wheel_speeds, robot_geometry robot_geometry) {
  float left_magnitude = (wheel_speeds.fl + wheel_speeds.rl) / 2;
  float right_magnitude = (wheel_speeds.fr + wheel_speeds.rr) / 2;

  float w_fl =
      wheel_speeds.fl * RPM_TO_RADS_SEC;
  float w_fr =
      wheel_speeds.fr * RPM_TO_RADS_SEC;
  float w_bl =
      wheel_speeds.rl* RPM_TO_RADS_SEC;
  float w_br =
      wheel_speeds.rr * RPM_TO_RADS_SEC;

  float lx = robot_geometry.intra_axle_distance / 2.0;
  float ly = robot_geometry.wheel_base / 2.0;

  /* compute velocities */
  float x_vel = (w_fl + w_fr + w_bl + w_br) * robot_geometry.wheel_radius / 4.0;
  float y_vel = (-w_fl + w_fr + w_bl - w_br) * robot_geometry.wheel_radius / 4.0;;
  float ang_vel = (-w_fl + w_fr - w_bl + w_br) * robot_geometry.wheel_radius / (4.0 * (lx + ly));

  robot_velocities returnstruct;
  returnstruct.linear_velocity = x_vel;
  returnstruct.trans_velocity = y_vel;
  returnstruct.angular_velocity = ang_vel;
  return returnstruct;
}
...
robot_velocities limitAcceleration(robot_velocities target_velocities,
  ...
  float linear_acceleration = (target_velocities.linear_velocity -
                              ...
                              dt;
  float trans_acceleration = (target_velocities.trans_velocity -
                               measured_velocities.trans_velocity) /
                              dt;
  ...
  if (std::abs(trans_acceleration) >
      std::abs(delta_v_limits.linear_velocity)) {
    std::signbit(trans_acceleration)
        ? trans_acceleration = -delta_v_limits.linear_velocity
        : trans_acceleration = delta_v_limits.linear_velocity;
  }
  ...
  return_velocities.linear_velocity = measured_velocities.linear_velocity + linear_acceleration * dt;
  return_velocities.trans_velocity = measured_velocities.trans_velocity + trans_acceleration * dt;
  // return_velocities.angular_velocity = measured_velocities.angular_velocity + angular_acceleration * dt;
...
robot_velocities scaleAngularCommand(robot_velocities target_velocities,
  ...
  return (robot_velocities){
      ...
      .trans_velocity = target_velocities.trans_velocity,
...
void MecanumMotionController::setTrim(float left_trim, float right_trim) {
  left_trim_value_ = left_trim;
  right_trim_value_ = right_trim;
}
float MecanumMotionController::getLeftTrim() {
  return left_trim_value_;
}
float MecanumMotionController::getRightTrim() {
  return right_trim_value_;
}
...
void MecanumMotionController::initializePids() {
  pid_mutex_.lock();
  switch (operating_mode_) {
    case OPEN_LOOP:
      break;
...
void MecanumMotionController::setAccelerationLimits(robot_velocities limits) {
  max_linear_acceleration_ = limits.linear_velocity;
  max_angular_acceleration_ = limits.angular_velocity;
}

robot_velocities MecanumMotionController::getAccelerationLimits() {
  robot_velocities returnstruct;
  returnstruct.angular_velocity = max_angular_acceleration_;
  returnstruct.linear_velocity = max_linear_acceleration_;
  return returnstruct;
...
void MecanumMotionController::setOperatingMode(
    robot_motion_mode_t operating_mode) {
  operating_mode_ = operating_mode;
  initializePids();
}

robot_motion_mode_t MecanumMotionController::getOperatingMode() {
  return operating_mode_;
}

void MecanumMotionController::setRobotGeometry(
    robot_geometry robot_geometry) {
  robot_geometry_ = robot_geometry;
}

robot_geometry MecanumMotionController::getRobotGeometry() {
  return robot_geometry_;
}

void MecanumMotionController::setPidGains(pid_gains pid_gains) {
  pid_gains_ = pid_gains;
}

pid_gains MecanumMotionController::getPidGains() { return pid_gains_; }

void MecanumMotionController::setMotorMaxDuty(float max_motor_duty) {
  max_motor_duty_ = max_motor_duty;
}
float MecanumMotionController::getMotorMaxDuty() { return max_motor_duty_; }

void MecanumMotionController::setMotorMinDuty(float min_motor_duty) {
  min_motor_duty_ = min_motor_duty;
}
float MecanumMotionController::getMotorMinDuty() { return min_motor_duty_; }

void MecanumMotionController::setOutputDecay(float geometric_decay) {
  geometric_decay_ = geometric_decay;
}
float MecanumMotionController::getOutputDecay() { return geometric_decay_; }

void MecanumMotionController::setOpenLoopMaxRpm(
    float open_loop_max_wheel_rpm) {
  open_loop_max_wheel_rpm_ = open_loop_max_wheel_rpm;
}
float MecanumMotionController::getOpenLoopMaxRpm() {
  return open_loop_max_wheel_rpm_;
}

void MecanumMotionController::setAngularScaling(
    angular_scaling_params angular_scaling_params) {
  angular_scaling_params_ = angular_scaling_params;
}

angular_scaling_params MecanumMotionController::getAngularScaling() {
  return angular_scaling_params_;
}

motor_data MecanumMotionController::computeMotorCommandsDual_(
    motor_data target_wheel_speeds, motor_data current_wheel_speeds) {
  /* average front and rear wheels */
  float left_magnitude =
      (current_wheel_speeds.fl + current_wheel_speeds.rl) / 2;
...
motor_data MecanumMotionController::computeMotorCommandsQuad_(
    motor_data target_wheel_speeds, motor_data current_wheel_speeds) {
  /* run pid, 1 per wheel */
  pid_mutex_.lock();
  pid_outputs fl_pid_output = pid_controller_fl_->runControl(
...
motor_data MecanumMotionController::clipDutyCycles_(
    motor_data proposed_duties) {
  /* clip extreme duty cycles in either direction (positive or negative) */
  proposed_duties.fr =
      std::clamp(proposed_duties.fr, -max_motor_duty_, max_motor_duty_);
...
motor_data MecanumMotionController::computeTorqueDistribution_(
    motor_data current_wheel_speeds, motor_data power_proposals) {
  /* right side */
  /* if both wheels are moving then ... */
...
robot_velocities MecanumMotionController::getMeasuredVelocities(
    motor_data current_wheel_speeds) {
  return computeMecanumVelocitiesFromWheelspeeds(current_wheel_speeds,
                                          robot_geometry_);
}

motor_data MecanumMotionController::runMotionControl(
    robot_velocities velocity_targets, motor_data current_duty_cycles,
    motor_data current_wheel_speeds) {
  /* take the time*/
  std::chrono::steady_clock::time_point time_now =
  ...
  /* get estimated robot velocities */
  measured_velocities_ =
      computeMecanumVelocitiesFromWheelspeeds(current_wheel_speeds, robot_geometry_);
  ...
  /* get target wheelspeeds from velocities */
  motor_data target_wheel_speeds =
      computeMecanumWheelSpeeds(velocity_commands, robot_geometry_);
