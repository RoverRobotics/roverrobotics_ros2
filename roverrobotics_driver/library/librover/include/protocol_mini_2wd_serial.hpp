...
class Mini2WDProtocolObject;
}

class RoverRobotics::Mini2WDProtocolObject : public RoverRobotics::BaseProtocolObject 
{
private:
  std::unique_ptr<Utilities::PersistentParams> persistent_params_;
  const std::string ROBOT_PARAM_PATH = std::string(std::getenv("HOME")) + "/robot.config";
  /* ---------- geometry & control ---------- */
  Control::robot_geometry robot_geometry_ = {.intra_axle_distance = 0.2659,  // robot_length (front-back axle spacing proxy)
                                             .wheel_base = 0.31,             // track width (center-to-center, left-right)
                                             .wheel_radius = 0.082760,         // 165mm diameter => 0.0825 m radius
                                             .center_of_mass_x_offset = 0,
                                             .center_of_mass_y_offset = 0};
  const float MOTOR_RPM_TO_WHEEL_RPM_RATIO_ = 4.83;
  const float OPEN_LOOP_MAX_RPM_ = 17000;

  const float MAX_CURVATURE_CORRECTION_ = 0.15;
  const int MOTOR_NEUTRAL_ = 0;
  const float MOTOR_MAX_ = 0.95;
  const float MOTOR_MIN_ = -0.95;
  const float LINEAR_JERK_LIMIT_ = 500;
...
public:
  Mini2WDProtocolObject(const char *device, std::string new_comm_type,
